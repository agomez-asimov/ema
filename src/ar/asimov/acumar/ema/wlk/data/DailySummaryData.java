...
public class DailySummaryData implements Serializable {
	...
	public static final DailySummaryData from(LocalDate date, DailySummary1 summary1, DailySummary2 summary2) {
		if (null == summary1)
			throw new IllegalArgumentException("DailySummary1 can't be null for DailySummaryData.from()");
		if (null == summary2)
			throw new IllegalArgumentException("DailySummary2 can't be null for DailySummaryData.from()");
		if (null == date)
			throw new IllegalArgumentException("Date can't be null for DailySummaryData.from()");
		...
		summary.setMaxAvg10MinWindSpeed(
				(Short.MIN_VALUE == summary1.hi10MinSpeed.get()) ? null : (summary1.hi10MinSpeed.get() / 1e1));
		...
		summary.setDailyUVDose(
				(Short.MIN_VALUE == summary1.dailyUVDose.get()) ? null : (summary1.dailyUVDose.get() / 1e1));
		...
		summary.setDailySolarEnergy(
				(Short.MIN_VALUE == summary2.dailySolarEnergy.get()) ? null : (summary2.dailySolarEnergy.get() / 1e1));
	...
	private static final LocalTime unpackTime(Unsigned8[] pack, int index) {
		...
		if (value == 0x0FFF || value == 0x07FF || value == 0x1000 || value == 0x0800)
			return null;
